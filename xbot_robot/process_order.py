...
def insert_into_database(order_id, intention_id, api_data):
    ...
    try:
        ...
        # 获取purpose_bill字段
        purpose_bill = order_info.get('purpose_bill', '')
        print(f"获取到的purpose_bill值: {purpose_bill}")
        ...
        with connection.cursor() as cursor:
            ...
            data = {
                ...
                'intention_code': purpose_bill  # 新增：将purpose_bill的值写入intention_code字段
            }
            ...
            print(f"intention_code值: {purpose_bill}")
            ...
            # 验证结果
            cursor.execute("SELECT ali_sku, intention_code FROM order_details WHERE order_id = %s", [order_id])
            result = cursor.fetchone()
            print(f"最终ali_sku值: {result[0] if result else 'None'}")
            print(f"最终intention_code值: {result[1] if result else 'None'}")
