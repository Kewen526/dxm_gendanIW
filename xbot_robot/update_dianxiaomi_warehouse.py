...
def update_dianxiaomi_warehouse(cookie_file_path, package_ids, storage_id):
    """
    从本地JSON文件读取cookie并更新包裹的仓库信息
    
    参数:
        cookie_file_path (str): 包含cookie的JSON文件路径
        package_ids (str): 包裹ID，多个ID可以用逗号分隔
        storage_id (str): 仓库ID
        
    返回:
        dict: API响应结果，成功时返回响应JSON，失败时返回None
    """
    # 从JSON文件读取cookie数据
    try:
    ...
    try:
        ...
        try:
            ...
            # 如果响应不是JSON格式，则返回文本内容
            print(f"解析响应JSON失败，响应内容: {response.text}")
...
if __name__ == "__main__":
    ...
    package_ids = "54280086909130128"  # 替换为您的packageIds
    storage_id = "6623995"  # 替换为您的storageId
    
    result = update_dianxiaomi_warehouse(cookie_file, package_ids, storage_id)
