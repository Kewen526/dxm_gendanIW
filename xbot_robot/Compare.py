...
"""
    ...
    for attempt in range(max_attempts):
        ...
        print(result)
        # Check if result contains the expected keyword
        if "识别结果" in result:
            # Clean the result by removing '#' characters
            result = result.replace("#", "")
            
            # Split the result by standard line breaks
            result_lines = result.split("\n")
            
            # Check if the first line indicates the products are the same
            if result_lines and "识别结果：一样" in result_lines[0]:
        ...
        print(f"Attempt {attempt+1} for {variant_key} did not return expected format, retrying...")
        if attempt == max_attempts - 1:
            print(f"Failed to get valid result for {variant_key} after {max_attempts} attempts")
...
def find_matching_variants(product_dict, reference_image_path, product_name):
    ...
    # List to store matching variants
    matching_variants = []
    
    # Process variants concurrently - use one worker per variant
    max_workers = len(product_dict)
    print(f"Processing {max_workers} variants concurrently")
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_variant):
            ...
            try:
                variant_key, is_same = future.result()
                if is_same:
                    matching_variants.append(variant_key)
                    print(f"Found matching variant: {variant_key}")
            except Exception as e:
                print(f"Error processing variant {variant_key}: {str(e)}")
